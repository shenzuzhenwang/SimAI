...
{
    int opt;
    while ((opt = getopt(argc, argv, "ht:w:g:s:n:c:x:")) != -1) // 新增 x
    {
        switch (opt)
        {
        case 'h':
        ...
        case 'c':
            ...
            break;
        case 'x': // 新增PS架构模式
            if (std::string(optarg) == "ps")
                user_param->ps_mode = true;
            break;
        default:
...
int main(int argc, char *argv[])
{
    ...
    /********************************************* */
    if (user_param.ps_mode)
    {
        double num_param = 7e9;     // Llama-7B
        double bytes_per_param = 4; // FP32
        double data_GB = num_param * bytes_per_param / (1024.0 * 1024.0 * 1024.0);
        double bandwidth_GBps = 400.0 / 8.0;            // 4x100Gbps
        double comm_time_ps = data_GB / bandwidth_GBps; // 单位秒

        sleep(100); // 等待日志系统初始化

        std::ofstream theory_out("ncclFlowModel_EndToEnd_PS.csv");
        theory_out << "total exposed comm," << std::fixed << std::setprecision(6) << comm_time_ps * 1e6 << std::endl; // 单位微秒
        theory_out.close();
        return 0;
    }
    /********************************************* */
