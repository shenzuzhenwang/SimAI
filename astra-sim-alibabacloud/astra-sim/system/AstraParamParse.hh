...
struct NetWorkParam
{
    ...
    char *nic_type = const_cast<char *>("cx7");
    bool visual = 0;
...
public:
    ...
    int parse(int argc, char *argv[])
    {
        for (int i = 1; i < argc; ++i)
        {
            ...
            {
                if (++i < argc)
                    this->workload = argv[i];
            }
            else if (arg == "-g" || arg == "--gpus")
            {
                if (++i < argc)
                    this->gpus.push_back(std::stoi(argv[i]));
            }
            ...
            {
                if (++i < argc)
                    this->res_folder = argv[i];
            }
            else if (arg == "-g_p_s" || arg == "--gpus-per-server")
            {
                if (++i < argc)
                    this->net_work_param.gpus_per_server = std::stoi(argv[i]);
            }
            else if (arg == "-nv" || arg == "--nvlink")
            {
                if (++i < argc)
                    this->net_work_param.nvlink_bw = std::stof(argv[i]);
            }
            else if (arg == "-nic" || arg == "--nic_busbw")
            {
                if (++i < argc)
                    this->net_work_param.bw_per_nic = std::stof(argv[i]);
            }
            else if (arg == "-n_p_s" || arg == "--nic_per_server")
            {
                if (++i < argc)
                    this->net_work_param.nics_per_server = std::stoi(argv[i]);
            }
            else if (arg == "-nic_t" || arg == "--nic_type")
            {
                if (++i < argc)
                    this->net_work_param.nic_type = argv[i];
            }
            else if (arg == "-g_type" || arg == "--gpu_type")
            {
                if (++i < argc)
                {
                    ...
                    if (gpu_type == "A100" || gpu_type == "a100")
                        this->net_work_param.gpu_type = GPUType::A100;
                    else if (gpu_type == "A800" || gpu_type == "a800")
                        this->net_work_param.gpu_type = GPUType::A800;
                    else if (gpu_type == "H100" || gpu_type == "h100")
                        this->net_work_param.gpu_type = GPUType::H100;
                    else if (gpu_type == "H800" || gpu_type == "h800")
                        this->net_work_param.gpu_type = GPUType::H800;
                    else if (gpu_type == "H20" || gpu_type == "h20")
                        this->net_work_param.gpu_type = GPUType::H20;
                    else
                        this->net_work_param.gpu_type = GPUType::NONE;
                }
            }
            else if (arg == "-v" || arg == "--visual")
            {
                if (++i < argc)
                    this->net_work_param.visual = std::stoi(argv[i]);
            }
            else if (arg == "--dp_overlap" || arg == "-dp_o")
            {
                if (++i < argc)
                    this->net_work_param.dp_overlap_ratio = std::stof(argv[i]);
            }
            else if (arg == "--tp_overlap" || arg == "-tp_o")
            {
                if (++i < argc)
                    this->net_work_param.tp_overlap_ratio = std::stof(argv[i]);
            }
            else if (arg == "--ep_overlap" || arg == "-ep_o")
            {
                if (++i < argc)
                    this->net_work_param.ep_overlap_ratio = std::stof(argv[i]);
            }
            else if (arg == "--pp_overlap" || arg == "-pp_o")
            {
                if (++i < argc)
                    this->net_work_param.pp_overlap_ratio = std::stof(argv[i]);
            }
            else
            {
                return 1;
        ...
        if (this->res == "None")
        {
            ...
            result << model_name << '-' << "tp" << tp << '-' << "pp" << pp << '-' << "dp" << dp << '-' << "ga" << static_cast<int>(ga) << '-' << "ep"
                   << ep << '-' << "NVL" << this->net_work_param.gpus_per_server << '-' << std::fixed << std::setprecision(1)
                   << (this->net_work_param.bw_per_nic * 8) << "G" << '-' << "DP" << this->net_work_param.dp_overlap_ratio << '-';
