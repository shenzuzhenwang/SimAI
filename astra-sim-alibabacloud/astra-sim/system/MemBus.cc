...
MemBus::MemBus(std::string side1, std::string side2, Sys *generator, Tick L, Tick o, Tick g, double G, bool model_shared_bus, int communication_delay,
               bool attach)
{
...
void MemBus::send_from_NPU_to_MA(MemBus::Transmition transmition, uint64_t bytes, bool processed, bool send_back, Callable *callable)
{
    ...
    else
    {
        ...
        {
            generator->register_event(callable, EventType::NPU_to_MA, new SharedBusStat(BusType::Shared, 0, communication_delay, 0, 0),
                                      communication_delay);
        }
    }
}
void MemBus::send_from_MA_to_NPU(MemBus::Transmition transmition,
                                 ...
                                 uint64_t bytes, bool processed, bool send_back, Callable *callable)
{
    MockNcclLog *NcclLog = MockNcclLog::getInstance();
    if (model_shared_bus && transmition == Transmition::Usual)
    {
    ...
    else
    {
        ...
        {
            generator->register_event(callable, EventType::MA_to_NPU, new SharedBusStat(BusType::Shared, 0, communication_delay, 0, 0),
                                      communication_delay);
        }
    }
}
