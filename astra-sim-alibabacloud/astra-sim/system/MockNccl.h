...
#define NCCL_TOPO_PATTERN_BALANCED_TREE                                                                                                              \
    1 // 目前正在使用的 Spread NIC traffic between two GPUs (Tree parent + one child on first GPU, second child on second GPU)
...
static const float baseLat[NCCL_NUM_ALGORITHMS][NCCL_NUM_PROTOCOLS] = {{6.8, 14.0, 0}, {6.6, 14.0, 8.4}, // Tree, Ring
...
template <typename X, typename Z = decltype(X() + int())> constexpr Z alignUp(X x, int a) { return (x + a - 1) & Z(-a); }
