    ...
    : g_flow_id(0), gpu_type(_gpu_type)
{
    ...
    if (TP_nums <= 0 || DP_nums <= 0 || PP_nums <= 0 || EP_nums <= 0 || DP_EP_nums <= 0 || (_TP_size * _DP_size * _PP_size != _ngpus) ||
        (_EP_size * _DP_EP_size != _DP_size))
    {
    ...
    if (_DP_size > 1)
    {
        ...
        for (int i = 0; i < DP_nums; i++)
        {
            ...
            AllGroups[all_group_idx] = GroupInfo(all_group_idx, DP, DPnodes.size(), _DP_size, ranks, NVSwitchs, _Dpus, _DpuNVSwitch);
            all_group_idx++;
        }
    }
    // init PP group
...
void MockNcclGroup::generateringchannels(std::map<int, std::vector<int>> localrings, MockNccl::GroupInfo *groupInfo,
...
std::map<int, std::vector<int>> MockNcclGroup::gen_local_ring(int rank, GroupType type)
{
    ...
    // printf("gen_local_ring(rank:%d,GroupType:%d)\n",rank,type);
    // gp_info.show();
    return localrings;
}
...
std::shared_ptr<void> MockNcclGroup::getFlowModels(GroupType type, int rank, AstraSim::ComType op, uint64_t data_size, int layer_num, State loopstate)
{
    ...
    {
        NcclLog->writeLog(NcclLogLevel::ERROR,
                          "There is no corresponding group info and group ring channel, resulting in an error in generating the flow model.");
        return nullptr;
    }
    gp_idx = GroupIndex[std::make_pair(rank, type)];
    gp_info = AllGroups[gp_idx];
    ...
    flow_model_name = flow_model_name + "_" + std::to_string(gp_idx) + "_" + std::to_string(layer_num) + "_" +
                      std::to_string(static_cast<int>(loopstate)) + "_" + std::to_string(static_cast<int>(op)) + "_" + std::to_string(data_size);
...
std::map<int, std::shared_ptr<FlowModels>> MockNcclGroup::genReduceScatterFlowModels(GroupType type, int rank, uint64_t data_size)
{
    FlowModels result = {};
    std::map<int, FlowModels> rank2flowmodels;
    std::map<int, std::shared_ptr<FlowModels>> rank2pflowmodels;
    ...
    for (auto it = ringchannels.begin(); it != ringchannels.end(); it++)
    {
        ...
        while (send_size < data_size)
        {
            ...
            for (auto rank_it = ring.begin(); rank_it != ring.end(); rank_it++)
            {
                ...
                if (curnoderecvrank != rank_it->second[2] && curnodesendrank != rank_it->second[3])
                {
                ...
                if (rank_it->second[3] == cur_rank && rank_it->second[2] != cur_rank && gp_info.nNodes > 1 && PXN_ENABLE)
                {
                    prevranks.clear();
                    if (rank_it->second[0] != -1)
                        prevranks = {rank_it->second[0]};
                    tmp_result = SingleFlow(g_flow_id, rank_it->first, rank_it->second[2], data_size, prevranks, {}, {g_flow_id + 1}, ring_id,
                                            chunkid, chunkcount, "RING");
                    ...
                    tmp_result = SingleFlow(g_flow_id, rank_it->second[2], rank_it->second[1], data_size, prevranks, {g_flow_id - 1}, {}, ring_id,
                                            chunkid, chunkcount, "PXN_INIT");
                    result[std::make_pair(ring_id, g_flow_id)] = tmp_result;
                    task_list[rank_it->first] = tmp_result;
                    g_flow_id++;
                }
                else if (rank_it->second[2] == cur_rank && rank_it->second[3] != cur_rank && gp_info.nNodes > 1 && PXN_ENABLE)
                {
                    prevranks.clear();
                    ...
                    tmp_result =
                        SingleFlow(g_flow_id, rank_it->first, rank_it->second[1], data_size, prevranks, {}, {}, ring_id, chunkid, chunkcount, "RING");
                    result[std::make_pair(ring_id, g_flow_id)] = tmp_result;
                    task_list[rank_it->first] = tmp_result;
                    g_flow_id++;
                }
            ...
            for (int i = 0; i < nranks - 2; i++)
            {
                ...
                for (auto rank_it = ring.begin(); rank_it != ring.end(); rank_it++)
                {
                    ...
                    if (rank_it->second[3] == cur_rank && rank_it->second[2] != cur_rank && gp_info.nNodes > 1 && PXN_ENABLE)
                    {
                        ...
                        tmp_result = SingleFlow(g_flow_id, rank_it->first, rank_it->second[2], data_size, prevranks, {partner_flow_id},
                                                {g_flow_id + 1}, ring_id, chunkid, chunkcount, "RING");
                    ...
                    else if (rank_it->second[2] == cur_rank && rank_it->second[3] != cur_rank && gp_info.nNodes > 1 && PXN_ENABLE)
                    {
                        ...
                        tmp_result = SingleFlow(g_flow_id, rank_it->first, rank_it->second[1], data_size, prevranks, {partner_flow_id}, {}, ring_id,
                                                chunkid, chunkcount, "RING");
                        result[std::make_pair(ring_id, partner_flow_id)].child_flow_id.push_back(g_flow_id);
                        task_list2[rank_it->first] = tmp_result;
                        result[std::make_pair(ring_id, g_flow_id)] = tmp_result;
...
std::map<int, std::shared_ptr<FlowModels>> MockNcclGroup::genAllReduceFlowModels(GroupType type, int rank, uint64_t data_size)
{
    ...
    {
    case NCCL_ALGO_DPU:
        return genAllReduceDpuFlowModels(type, rank, data_size);
    case NCCL_ALGO_TREE:
    case NCCL_ALGO_RING:
...
std::shared_ptr<FlowModels> MockNcclGroup::genallReduceNVLSTreeFlowModels(GroupType type, int rank, uint64_t data_size)
{
    MockNcclLog *NcclLog = MockNcclLog::getInstance();
    GroupInfo gp_info;
    int gp_idx;
    ...
    {
        NcclLog->writeLog(NcclLogLevel::ERROR,
                          "There is no relevant group info, resulting in an error in generating genallReduceNVLSTreeFlowModels.");
        return nullptr;
    }
    gp_idx = GroupIndex[std::make_pair(rank, type)];
    gp_info = AllGroups[gp_idx];
    ...
    for (nvlstree = nvlstreechannels.begin(); nvlstree != nvlstreechannels.end(); nvlstree++)
    {
        std::map<int, std::vector<ncclChannelNode *>>::iterator nvlstreenodes_it;
        if (rank == 0)
        {
            for (nvlstreenodes_it = nvlstree->second.begin(); nvlstreenodes_it != nvlstree->second.end(); nvlstreenodes_it++)
            {
        ...
        for (int ck = 0; ck < chunk_count; ck++)
        {
            ...
            generate_flow_model_nvls_tree_allreduce_up(ncclchannelnodes, upinDegree, nodeprevs, chunk_size, ck, chunk_count, nvlstree->first, result);
            generate_flow_model_nvls_tree_allreduce_down(ncclchannelnodes, downinDegree, nodeprevs, chunk_size, ck, chunk_count, nvlstree->first,
                                                         result);
        }
    }
...
FlowModels MockNcclGroup::generate_flow_model_nvls_tree_allreduce_up(std::vector<ncclChannelNode *> nvlstreenodes,
                                                                     std::unordered_map<ncclChannelNode *, int> upinDegree,
                                                                     std::unordered_map<ncclChannelNode *, std::vector<int>> &nodeprevs,
                                                                     int chunk_size, int chunk_id, int chunk_count, int channle_id,
                                                                     FlowModels &result)
{
    ...
    while (!q.empty())
    {
        ...
        if (current->up != nullptr)
        {
            ...
            tmp_result = SingleFlow(g_flow_id, current->rank, current->up->rank, chunk_size, _prev, nodeprevs[current], {}, channle_id, chunk_id,
                                    chunk_count, conn_tag);
...
FlowModels MockNcclGroup::generate_flow_model_nvls_tree_allreduce_down(std::vector<ncclChannelNode *> nvlstreenodes,
                                                                       std::unordered_map<ncclChannelNode *, int> downinDegree,
                                                                       std::unordered_map<ncclChannelNode *, std::vector<int>> &nodeprevs,
                                                                       int chunk_size, int chunk_id, int chunk_count, int channle_id,
                                                                       FlowModels &result)
    ...
    while (!q.empty())
    {
        ...
        if (current->down.size() > 0)
        {
            for (ncclChannelNode *down : current->down)
            {
                ...
                tmp_result = SingleFlow(g_flow_id, current->rank, down->rank, chunk_size, _prev, nodeprevs[current], {}, channle_id, chunk_id,
                                        chunk_count, conn_tag);
                for (int parent_flow_id : nodeprevs[current])
                {
                    result[std::make_pair(channle_id, parent_flow_id)].child_flow_id.push_back(g_flow_id);
...
FlowModels MockNcclGroup::generate_flow_model_tree_allreduce_up(std::map<int, ncclTree> &nodes, std::unordered_map<int, int> upinDegree,
                                                                std::unordered_map<int, std::vector<int>> &nodeprevs, int chunk_size, int chunk_id,
                                                                int chunk_count, int channle_id, FlowModels &result)
{
    ...
    while (!q.empty())
    {
        ...
        if (current.up != -1)
        {
            ...
            if (current.down.size() == 0)
                ...
                _prev = current.down;
            tmp_result = SingleFlow(g_flow_id, current.rank, current.up, chunk_size, _prev, nodeprevs[current.rank], {}, channle_id, chunk_id,
                                    chunk_count, conn_tag);
...
FlowModels MockNcclGroup::generate_flow_model_tree_allreduce_down(std::map<int, ncclTree> &nodes, std::unordered_map<int, int> downinDegree,
                                                                  std::unordered_map<int, std::vector<int>> &nodeprevs, int chunk_size, int chunk_id,
                                                                  int chunk_count, int channle_id, FlowModels &result)
{
    std::queue<ncclTree> q;
    ...
    while (!q.empty())
    {
        ...
        if (current.down.size() > 0)
        {
            for (int down : current.down)
            {
                ...
                tmp_result = SingleFlow(g_flow_id, current.rank, down, chunk_size, _prev, nodeprevs[current.rank], {}, channle_id, chunk_id,
                                        chunk_count, conn_tag);
                for (int parent_flow_id : nodeprevs[current.rank])
                    result[std::make_pair(channle_id, parent_flow_id)].child_flow_id.push_back(g_flow_id);
                result[std::make_pair(channle_id, g_flow_id)] = tmp_result;
...
FlowModels MockNcclGroup::genAllReduceMultiDpuFlowModels(GroupInfo gp_info, uint64_t data_size, std::vector<int> dpus)
{
    ...
    // num_dpus *2=0.8462 | num_dpus *4=0.706 | *8=0.6362 | *16=0.6012 *64=0.5840 *1024=0.5673 *2048=0.5668 *4096=0.5665 理论下限 0.56
    ...
    std::unordered_map<int, int> dpuchannel; // channel 映射 dpu
    for (int i = 0; i < dpus.size(); ++i)
        dpuchannel[dpus[i]] = i;

    // 平均分 chunk 给 DPU：前 extra_chunks 个 DPU 多分 1 个
    int base_chunks = num_chunks / num_dpus;
    int extra_chunks = num_chunks % num_dpus;
    int chunk_counter = 0;

    // 遍历每个 DPU 分配它负责的 chunk
    for (int dpu_index = 0; dpu_index < num_dpus; ++dpu_index)
    {
        int dpuId = dpus[dpu_index];
        int chunks_for_dpu = base_chunks + (dpu_index < extra_chunks ? 1 : 0);

        for (int local_chunk_idx = 0; local_chunk_idx < chunks_for_dpu; ++local_chunk_idx, ++chunk_counter)
        {
            // 分配 flow IDs
            std::vector<int> send_ids(num_gpus), recv_ids(num_gpus);
            for (int i = 0; i < num_gpus; ++i)
            {
                send_ids[i] = g_flow_id++;
                recv_ids[i] = g_flow_id++;
            }

            // GPU → DPU 上传
            for (int gpu_id = 0; gpu_id < num_gpus; ++gpu_id)
            {
                result[{dpuchannel[dpuId], send_ids[gpu_id]}] = SingleFlow(send_ids[gpu_id],  // flow_id
                                                                           gpu_id,            // src
                                                                           dpuId,             // dst (负责聚合的DPU)
                                                                           agg_grain,         // 数据量
                                                                           {dpuId},           // 无前驱
                                                                           {},                // parent
                                                                           recv_ids,          // child: 对应本chunk所有recv_flow
                                                                           dpuchannel[dpuId], // channel_id
                                                                           local_chunk_idx,   // chunk_id
                                                                           chunks_for_dpu,    // 总chunk数
                                                                           "GPU2DPU");
            }
            ...
            for (int gpu_id = 0; gpu_id < num_gpus; ++gpu_id)
            {
                result[{dpuchannel[dpuId], recv_ids[gpu_id]}] = SingleFlow(recv_ids[gpu_id],  // flow_id
                                                                           dpuId,             // src
                                                                           gpu_id,            // dst
                                                                           agg_grain,         // 数据量
                                                                           all_gpu_ids,       // 前驱：所有GPU
                                                                           send_ids,          // parent: 所有send_flow
                                                                           {},                // child
                                                                           dpuchannel[dpuId], // channel_id
                                                                           local_chunk_idx,   // chunk_id
                                                                           chunks_for_dpu,    // 总chunk数
        ...
        while (send_size < data_size)
        {
            ...
            for (auto rank_it = ring.begin(); rank_it != ring.end(); rank_it++)
            {
                ...
                if (rank_it->second[3] == cur_rank && rank_it->second[2] != cur_rank && gp_info.nNodes > 1 && PXN_ENABLE)
                {
                    ...
                    tmp_result = SingleFlow(g_flow_id, rank_it->first, rank_it->second[2], data_size, prevranks, {}, {g_flow_id + 1}, ring_id,
                                            chunk_id, chunkcout, "RING");
                    result[std::make_pair(ring_id, g_flow_id)] = tmp_result;
                    g_flow_id++;
                    prevranks.clear();
                    prevranks = {rank_it->first};
                    tmp_result = SingleFlow(g_flow_id, rank_it->second[2], rank_it->second[1], data_size, prevranks, {g_flow_id - 1}, {}, ring_id,
                                            chunk_id, chunkcout, "PXN_INIT");
                    result[std::make_pair(ring_id, g_flow_id)] = tmp_result;
                    task_list[rank_it->first] = tmp_result;
                    g_flow_id++;
                }
                else if (rank_it->second[2] == cur_rank && rank_it->second[3] != cur_rank && gp_info.nNodes > 1 && PXN_ENABLE)
                {
                    ...
                    tmp_result =
                        SingleFlow(g_flow_id, rank_it->first, rank_it->second[1], data_size, prevranks, {}, {}, ring_id, chunk_id, chunkcout, "RING");
                    result[std::make_pair(ring_id, g_flow_id)] = tmp_result;
                    task_list[rank_it->first] = tmp_result;
                    g_flow_id++;
                }
            ...
            for (int i = 0; i < nranks - 1; i++)
            {
                ...
                for (auto rank_it = ring.begin(); rank_it != ring.end(); rank_it++)
                {
                    ...
                    if (rank_it->second[3] == cur_rank && rank_it->second[2] != cur_rank && gp_info.nNodes > 1 && PXN_ENABLE)
                    {
                        ...
                        tmp_result = SingleFlow(g_flow_id, rank_it->first, rank_it->second[2], data_size, prevranks, {partner_flow_id},
                                                {g_flow_id + 1}, ring_id, chunk_id, chunkcout, "RING");
                        result[std::make_pair(ring_id, partner_flow_id)].child_flow_id.push_back(g_flow_id);
                        result[std::make_pair(ring_id, g_flow_id)] = tmp_result;
                        g_flow_id++;
                        prevranks.clear();
...
TreeChannels MockNcclGroup::getdpuchannels(int rank, GroupType type)
{
    ...
    {
        NcclLog->writeLog(NcclLogLevel::ERROR,
                          "There is no corresponding group info and group ring channel, resulting in an error in get_nvls_channels.");
        return {};
    }
    gp_idx = GroupIndex[std::make_pair(rank, type)];
    gp_info = AllGroups[gp_idx];

    std::unordered_map<int, int> dpuchannel_map;
    for (int i = 0; i < gp_info.Dpus.size(); i++)
        dpuchannel_map[gp_info.Dpus[i]] = i;

    TreeChannels dpuchannel;
    for (auto dpuId : gp_info.Dpus)
    {
        vector<int> _down;
        for (int i = 0; i < gp_info.nRanks; i++)
        {
            dpuchannel[dpuchannel_map[dpuId]][i] = ncclTree(-1, i, dpuId, {});
            _down.push_back(i);
        }
        dpuchannel[dpuchannel_map[dpuId]][dpuId] = ncclTree(-1, dpuId, -1, _down);
    }
    return dpuchannel;
}
...
NVLStreechannels MockNcclGroup::get_nvls_tree_channels(int rank, GroupType type)
{
    ...
    std::map<int, std::map<int, std::vector<int>>> allnode2ranks;
    for (ring_it = rings.begin(); ring_it != rings.end(); ring_it++)
    {
        ...
        for (int i = 0; i < gp_info.nNodes; i++)
        {
            ...
            {
                allnode2ranks[ring_it->first][i].push_back(ring_it->second[i * nrankspernode + j]);
            }
        }
    }
    ...
    for (DoubleBinaryTreeNode *root : roots)
    {
        for (int index = 0; index < nlocalRanks; index++)
        {
            ...
            for (int i = 0; i < nNodes; i++)
            {
                ...
                intra_topo.insert(intra_topo.end(), noderanks.begin(), noderanks.end());
                NcclLog->writeLog(NcclLogLevel::DEBUG, " node  %d intra_topo", i);
                for (auto num : intra_topo)
                {
                    NcclLog->writeLog(NcclLogLevel::DEBUG, " %d", num);
                }
                ncclChannelNode *root = gen_nvls_tree_intra_channels(intra_topo, nvlstreechannel);
            ...
            if (rank == 0)
            {
                for (nvlstreenodes_it = nvlstreechannel.begin(); nvlstreenodes_it != nvlstreechannel.end(); nvlstreenodes_it++)
                {
                    NcclLog->writeLog(NcclLogLevel::DEBUG, " rank  %d nvls tree nodes ", nvlstreenodes_it->first);
                    int i = 0;
                    for (auto nvlstreenode : nvlstreenodes_it->second)
            ...
            gen_nvls_tree_inter_channels(root, nodencclchannlenodes, nvlstreechannel);
...
ncclChannelNode *MockNcclGroup::gen_nvls_tree_inter_channels(DoubleBinaryTreeNode *root, std::map<int, ncclChannelNode *> nodencclchannlenodes,
                                                             std::map<int, vector<ncclChannelNode *>> &nvlstreechannel)
{
    MockNcclLog *NcclLog = MockNcclLog::getInstance();
    if (root == nullptr)
...
TreeChannels MockNcclGroup::gettreechannels(int rank, GroupType type)
{
    ...
    {
        NcclLog->writeLog(NcclLogLevel::ERROR,
                          "There is no corresponding group info and group ring channel, resulting in an error in gettreechannels.");
        return {};
    }
    gp_idx = GroupIndex[std::make_pair(rank, type)];
    gp_info = AllGroups[gp_idx];
    ...
    std::map<int, std::map<int, std::vector<int>>>::iterator allnode2ranks_it;
    int channel_id = 0;
    for (allnode2ranks_it = allnode2ranks.begin(); allnode2ranks_it != allnode2ranks.end(); allnode2ranks_it++)
    {
...
ncclInfo *MockNcclGroup::get_algo_proto_info(GroupType type, int rank, AstraSim::ComType op, uint64_t data_size)
{
    ...
    else
    {
        ...
        case AstraSim::ComType::All_Reduce:
            ...
            else if (gp_info.Dpus.size() > 0)
            {
                info->algorithm = NCCL_ALGO_DPU;
            }
            else
            {
                info->algorithm = NCCL_ALGO_RING;
