...
struct TuneInfo
{
    ...
    TuneInfo() {};
    ~TuneInfo() {};
    TuneInfo(int _nNodes, int _nRanks, int _nChannels, int _collNetSupport, int _nvlsSupport, int _minCompCap, int _maxCompCap)
        : nNodes(_nNodes), nRanks(_nRanks), nChannels(_nChannels), collNetSupport(_collNetSupport), nvlsSupport(_nvlsSupport),
          minCompCap(_minCompCap), maxCompCap(_maxCompCap)
    {
        graphs = std::vector<ncclTopoGraph *>(NCCL_NUM_ALGORITHMS, nullptr);
        latencies = std::vector<std::vector<std::vector<float>>>(
            NCCL_NUM_FUNCTIONS, std::vector<std::vector<float>>(NCCL_NUM_ALGORITHMS, std::vector<float>(NCCL_NUM_PROTOCOLS, 0)));
...
public:
    ...
    inline void show() const
    {
        ...
        for (const auto &dpu : Dpus)
            std::cout << dpu << " ";
        std::cout << "  DpuNVSwitch: ";
        for (const auto &sw : DpuNVSwitch)
            std::cout << sw << " ";
...
public:
    MockNcclGroup() {}
    MockNcclGroup(int _ngpus, int _gpus_per_nodes, int _TP_size, int _DP_size, int _PP_size, int _EP_size, int _DP_EP_size,
                  std::vector<int> _NVSwitch, GPUType _gpu_type, std::vector<int> _Dpus = {}, int _dpu_per_sw = 0,
                  std::vector<int> _DpuNVSwitch = {});
    ~MockNcclGroup() {};
...
private:
    ...
    FlowModels generate_flow_model_nvls_tree_allreduce_up(std::vector<ncclChannelNode *> nvlstreenodes,
                                                          std::unordered_map<ncclChannelNode *, int> upinDegree,
                                                          std::unordered_map<ncclChannelNode *, std::vector<int>> &nodeprevs, int chunk_size,
                                                          int chunk_id, int chunk_count, int channle_id, FlowModels &result);
    FlowModels generate_flow_model_nvls_tree_allreduce_down(std::vector<ncclChannelNode *> nvlstreenodes,
    ...
    FlowModels generate_flow_model_tree_allreduce_up(std::map<int, ncclTree> &nodes, std::unordered_map<int, int> upinDegree,
    ...
    FlowModels generate_flow_model_tree_allreduce_down(std::map<int, ncclTree> &nodes, std::unordered_map<int, int> downinDegree,
                                                       std::unordered_map<int, std::vector<int>> &nodeprevs, int chunk_size, int chunk_id,
                                                       int chunk_count, int channle_id, FlowModels &result);
...
public:
    void generateringchannels(std::map<int, std::vector<int>> localrings, MockNccl::GroupInfo *groupInfo,
    ...
    ncclChannelNode *gen_nvls_tree_inter_channels(DoubleBinaryTreeNode *root, std::map<int, ncclChannelNode *> nodencclchannlenodes,
                                                  std::map<int, vector<ncclChannelNode *>> &nvlstreechannel);
    ncclInfo *get_algo_proto_info(GroupType type, int rank, AstraSim::ComType op, uint64_t data_size);
};
}
#endif