...
constexpr int Print_ID = -1; // 打印日志的节点ID，-1表示打印所有
...
private:
    ...
    MockNcclLog()
    {
        ...
        logLevel = logLevelEnv ? static_cast<NcclLogLevel>(std::atoi(logLevelEnv)) : NcclLogLevel::INFO;
...
public:
    ...
    static void set_log_name(std::string log_name) { LogName = LOG_PATH + log_name; }
