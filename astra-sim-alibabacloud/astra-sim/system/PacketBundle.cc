...
namespace AstraSim
{
PacketBundle::PacketBundle(Sys *generator, BaseStream *stream, std::list<MyPacket *> locked_packets, bool needs_processing, bool send_back,
                           uint64_t size, MemBus::Transmition transmition)
{
...
PacketBundle::PacketBundle(Sys *generator, BaseStream *stream, std::list<MyPacket *> locked_packets, bool needs_processing, bool send_back,
                           uint64_t size, MemBus::Transmition transmition, int channel_id, int flow_id)
{
    this->generator = generator;
    this->locked_packets = locked_packets;
    this->needs_processing = needs_processing;
...
PacketBundle::PacketBundle(Sys *generator, BaseStream *stream, bool needs_processing, bool send_back, uint64_t size, MemBus::Transmition transmition)
{
    this->generator = generator;
    this->needs_processing = needs_processing;
    this->send_back = send_back;
...
void PacketBundle::send_to_MA() { generator->memBus->send_from_NPU_to_MA(transmition, size, needs_processing, send_back, this); }
...
void PacketBundle::call(EventType event, CallData *data)
{
    ...
    if (channel_id == -1)
        stream->call(EventType::General, data); // 走这里
    else
        stream->call(EventType::NCCL_General, ehd);
    delete this;
}
} // namespace AstraSim
