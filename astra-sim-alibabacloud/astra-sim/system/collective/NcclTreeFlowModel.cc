...
NcclTreeFlowModel::NcclTreeFlowModel(ComType type, int id, int layer_num, RingTopology *ring_topology, uint64_t data_size,
                                     RingTopology::Direction direction, InjectionPolicy injection_policy, bool boost_mode,
                                     std::shared_ptr<MockNccl::FlowModels> ptr_flow_models, int treechannels)
    : Algorithm(layer_num)
{
...
int NcclTreeFlowModel::get_non_zero_latency_packets() { return (nodes_in_ring - 1) * parallel_reduce * 1; }
...
void NcclTreeFlowModel::run(EventType event, CallData *data)
{
    ...
    else if (event == EventType::StreamInit)
    {
        ...
        for (auto single_flow : _flow_models)
        {
            ...
            {
#ifdef PHY_RDMA
                flow_rdma.ibv_create_peer_qp(id, single_flow.second.channel_id, single_flow.second.src, single_flow.second.dest,
                                             single_flow.second.chunk_count + 1, single_flow.second.chunk_id, single_flow.second.flow_size);
#endif
            }
        }
        MPI_Barrier(MPI_COMM_WORLD);
            ...
            for (int j = 0; j < m_channels; j++)
            {
                for (const auto flow_model : _flow_models)
                {
                    ...
                    if ((parent_list.size() == 0) && flow_model.second.channel_id == j)
                    {
                        ...
                        if (flow_model.second.chunk_id == 0)
                        {
                            pQps->peer_qps[std::make_pair(flow_model.second.channel_id,
                                                          std::make_pair(flow_model.second.src, flow_model.second.dest))] = 0;
                            insert_packets(j, flow_model.second.flow_id);
                        }
                        else
                        {
                            pQps->peer_wating_tasks[std::make_pair(flow_model.second.channel_id,
    ...
    else if (event == EventType::PacketSentFinshed)
    {
        ...
        {
            int cur_flow_id =
                pQps->peer_wating_tasks[std::make_pair(flowTag.channel_id, std::make_pair(flowTag.sender_node, flowTag.receiver_node))].front();
...
bool NcclTreeFlowModel::recv_ready(int channel_id, int flow_id)
{
    ...
    for (int recv_prev : recv_prevs)
    {
        ...
        RecvPacketEventHadndlerData *ehd = new RecvPacketEventHadndlerData(stream, stream->owner->id, EventType::PacketReceived, recv_prev, 1);
        ...
        stream->owner->front_end_sim_recv(0, Sys::dummy_data, _flow_models[std::make_pair(channel_id, flow_id)].flow_size, UINT8, recv_prev,
                                          channel_id, &rcv_req, &Sys::handleEvent, ehd);
    }
    return true;
}
...
bool NcclTreeFlowModel::ready(int channel_id, int flow_id)
{
    ...
    for (int recv_prev : recv_prevs)
    {
        ...
        RecvPacketEventHadndlerData *ehd =
            new RecvPacketEventHadndlerData(stream, stream->owner->id, EventType::PacketReceived, packet.preferred_vnet, packet.stream_num);
        ehd->flowTag.child_flow_id = -1;
        ehd->flowTag.current_flow_id = -1;
        ...
        {
            ehd->flowTag.tag_id =
                layer_num * flow_model.chunk_count * m_channels + flow_model.chunk_count * flow_model.channel_id + flow_model.chunk_id;
        }
        else
        {
            ehd->flowTag.tag_id =
                layer_num * flow_model.chunk_count * m_channels + flow_model.chunk_count * flow_model.channel_id + flow_model.chunk_id + 1;
        }
        ehd->flowTag.channel_id = packet.channel_id;
        if (free_packets[std::make_pair(channel_id, recv_prev)] > 0)
        {
            stream->owner->front_end_sim_recv(0, Sys::dummy_data, rcv_req.reqCount, UINT8, recv_prev, rcv_req.tag, &rcv_req, &Sys::handleEvent, ehd);
        }
    }
    sim_request snd_req;
    snd_req.srcRank = id;
    ...
    snd_req.flowTag.tag_id = layer_num * flow_model.chunk_count * m_channels + flow_model.channel_id * flow_model.chunk_count + flow_model.chunk_id;
    ...
    SendPacketEventHandlerData *send_ehd =
        new SendPacketEventHandlerData(stream, id, packet.preferred_dest, channel_id, EventType::PacketSentFinshed);
    stream->owner->front_end_sim_send(0, Sys::dummy_data, snd_req.reqCount, UINT8, packet.preferred_dest, snd_req.flowTag.tag_id, &snd_req,
                                      &Sys::handleEvent, send_ehd);
    return true;
}
...
bool NcclTreeFlowModel::phy_ready(int channel_id, int flow_id)
{
    ...
    for (int recv_prev : recv_prevs)
    {
        ...
        rcv_req.tag = channel_id;
        RecvPacketEventHadndlerData *ehd =
            new RecvPacketEventHadndlerData(stream, stream->owner->id, EventType::PacketReceived, stream->current_queue_id, 1);
        ehd->flowTag.child_flow_id = -1;
        ehd->flowTag.current_flow_id = -1;
        auto flow_model = this->_flow_models[std::make_pair(channel_id, flow_id)];
        if (flow_model.parent_flow_id.size() == 0 || flow_model.conn_type == "RING")
    ...
    if (this->comType == ComType::All_Reduce_NVLS)
        ...
        snd_req.flowTag.nvls_on = false;
    SendPacketEventHandlerData *send_ehd = new SendPacketEventHandlerData(stream, id, flow.dest, channel_id, EventType::PacketSentFinshed);
    stream->owner->front_end_sim_send(0, Sys::dummy_data, snd_req.reqCount, UINT8, flow.dest, snd_req.tag, &snd_req, &Sys::handleEvent, send_ehd);
    return true;
}
