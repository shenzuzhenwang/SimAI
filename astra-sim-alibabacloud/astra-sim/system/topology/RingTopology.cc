    ...
    : BasicLogicalTopology(BasicLogicalTopology::BasicTopology::Ring)
{
    ...
    if (id == 0)
    {
        std::cout << "ring of node 0, "
                  << "id: " << id << " dimension: " << name << " total nodes in ring: " << total_nodes_in_ring << " index in ring: " << index_in_ring
                  << " offset: " << offset << "total nodes in ring: " << total_nodes_in_ring << std::endl;
    }
    this->id = id;
...
int RingTopology::get_receiver_node(int node_id, Direction direction)
{
    ...
    if (direction == RingTopology::Direction::Clockwise)
    {
        ...
        if (receiver < 0)
        {
            std::cout << "at dim: " << name << "at id: " << id << "dimension: " << name << " index: " << index << " ,node id: " << node_id
                      << " ,offset: " << offset << " ,index_in_ring: " << index_in_ring << " receiver: " << receiver << std::endl;
        }
        assert(receiver >= 0);
...
int RingTopology::get_sender_node(int node_id, Direction direction)
{
    ...
    if (direction == RingTopology::Direction::Anticlockwise)
    {
        ...
        if (sender < 0)
        {
            std::cout << "at dim: " << name << " at id: " << id << " index: " << index << " ,node id: " << node_id << " ,offset: " << offset
                      << " ,index_in_ring: " << index_in_ring << " ,sender: " << sender << std::endl;
        }
        assert(sender >= 0);
...
int RingTopology::get_num_of_nodes_in_dimension(int dimension) { return get_nodes_in_ring(); }
} // namespace AstraSim
