...
void Workload::report()
{
    ...
    {
#ifdef ANALYTI
        astraSimDataAPI.layers_stats.push_back(layers[i]->report(run_name, i, total_rows, stat_row, detailed, end_to_end, total_compute,
                                                                 total_exposed, pre_bubble_time, DP_comm, DP_EP_comm, Expose_TP_comm, Expose_EP_comm,
                                                                 this->seprate_log));
#else
        astraSimDataAPI.layers_stats.push_back(layers[i]->report(run_name, i, total_rows, stat_row, detailed, end_to_end, total_compute,
                                                                 total_exposed, this->seprate_log, total_fwd_time, total_wg_time, total_ig_time,
...
void Workload::iterate_hybrid_parallel_customized()
{
    ...
    else if (current_state == LoopState::Input_Gradient)
    {
        ...
        {
            collective_issued = true;
            layers[index]->issue_input_grad_comm(SchedulingPolicy::LIFO, CollectiveBarrier::Non_Blocking);
        }
        collective_issued = false;
        delay_loaded = false;
        current_state = LoopState::Weight_Gradient;
...
void Workload::iterate_hybrid_parallel_Transformer_fwd_in_bckwd()
{
    ...
    assert(index >= 0);
    assert(index < SIZE);
    check_for_sim_end(); // 检查是否所有pass都完成，决定是否结束仿真
    ...
    if (current_state == LoopState::Forward_Pass)
    {
        ...
        delay_loaded = false;
        collective_issued = false;
        // 如果所有层都完成，进入反向传播输入梯度阶段（下面第二个else if）
        if (index >= SIZE)
        {
            current_state = LoopState::Input_Gradient;
            index--;
    ...
    else if (current_state == LoopState::Weight_Gradient)
    {
        ...
        // 等待反向输入梯度通信完成；即，如果当前层的输入梯度通信还没有完成，则started_waiting_for_input_grad中记录现在的仿真时间
        if (!layers[index]->is_input_grad_comm_finished_blocking())
        {
            return;
        }
    ...
    {
        // 检查是否需要进入“前向在反向”阶段（checkpoint机制）
        if (layers[index]->needs_fwd_in_bckwd_initiation && !checkpoint_initiated)
        {
            ...
            if (generator->id == 0)
            {
                std::cout << "***** info, initiating fwd_in_bkwd starting from layer:" << index << " to layer: " << tmp
    ...
    // 前向在反向阶段（用于Transformer模型的checkpoint机制）
    else if (current_state == LoopState::Forward_In_BackPass)
    {
        ...
        NcclLog->writeLog(NcclLogLevel::DEBUG, "workload %d::call fwd_in_bckwd index: %d register_event EventType::General %d", generator->id, index,
                          1);
        generator->register_event(this, EventType::General, NULL, 1);
        return;
    }
...
void Workload::iterate_hybrid_parallel_DLRM()
{
    ...
    if (current_state == LoopState::Forward_Pass)
    {
        ...
        {
            collective_issued = true;
            layers[index]->issue_forward_pass_comm(SchedulingPolicy::HIGHEST, CollectiveBarrier::Non_Blocking);
        }
    ...
    else if (current_state == LoopState::Input_Gradient)
    {
        ...
        {
            layers[0]->issue_input_grad_comm(SchedulingPolicy::HIGHEST, CollectiveBarrier::Non_Blocking);
        }
        index--;
        if (generator->id == 0)
        {
...
// 根据并行策略（ParallelismPolicy）判断模型各阶段（前向 fwd、输入梯度 ig、权重梯度 wg）在哪些维度上发生通信或参与分布式计算。
// 一维的话，都为true
std::map<std::string, std::vector<bool>> Workload::decode_involved_dimensions(ParallelismPolicy policy, int model_parallel_npu_group)
{
    ...
    std::vector<bool> none{false, false, false, false, false, false, false, false, false, false};
    std::vector<bool> all{true, true, true, true, true, true, true, true, true, true};
    ...
    else if (policy == ParallelismPolicy::Data || policy == ParallelismPolicy::DLRM || policy == ParallelismPolicy::DLRMEnhanced ||
    ...
    else if (policy == ParallelismPolicy::Model || policy == ParallelismPolicy::DistributedInference)
    {
        result["fwd"] = all;
        result["ig"] = all;
        result["wg"] = none;
    ...
    {
        std::vector<bool> data{true, false, false, false, false, false, false, false, false, false};
        std::vector<bool> model{false, true, true, true, true, true, true, true, true, true};
        result["fwd"] = model;
        result["ig"] = model;
        result["wg"] = data;
    }
    ...
    {
        std::vector<bool> model{true, false, false, false, false, false, false, false, false, false};
        std::vector<bool> data{false, true, true, true, true, true, true, true, true, true};
        result["fwd"] = model;
        result["ig"] = model;
        result["wg"] = data;
    }
    else if (policy == ParallelismPolicy::TransformerFwdInBckwd || policy == ParallelismPolicy::Transformer)
    {
...
bool Workload::initialize_workload(std::string name)
{
    ...
    if (parallelismPolicy == ParallelismPolicy::TransformerFwdInBckwd || parallelismPolicy == ParallelismPolicy::Transformer)
    {
    ...
    std::map<std::string, std::vector<bool>> general_involved_dimensions = decode_involved_dimensions(parallelismPolicy, model_parallel_npu_group);
    pp_commsize = 0;
    for (size_t i = 1; i < tokens.size(); i = i + 1)
    {
    ...
    for (int i = 0; i < lines; i++)
    {
        ...
        Tick fp_compute_time; // forward compute
        inFile >> fp_compute_time;
        std::string fp_comm_type_s; // forward communication type   执行张量并行（TP）的前向通信，例如 ALLGATHER。
        inFile >> fp_comm_type_s;
        uint64_t fp_comm_size; // forward communication size
        inFile >> fp_comm_size;

        Tick ig_compute_time; // input gradient compute
        inFile >> ig_compute_time;
        std::string ig_comm_type_s; // input gradient communication type    进行反向通信（TP组），如 ALLREDUCE。
        inFile >> ig_comm_type_s;
        uint64_t ig_comm_size; // input gradient communication size
        inFile >> ig_comm_size;

        Tick wg_compute_time; // weight gradient compute
        inFile >> wg_compute_time;
        std::string wg_comm_type_s; // weight gradient communication type   进行数据并行（DP组）通信，通常是 REDUCESCATTER，用于跨 GPU 聚合梯度。
        inFile >> wg_comm_type_s;
        uint64_t wg_comm_size; // weight gradient communication size
        inFile >> wg_comm_size;

        Tick wg_update_time; // weight gradient update time
        ...
        if ((parallelismPolicy == ParallelismPolicy::DLRM || parallelismPolicy == ParallelismPolicy::DLRMEnhanced) && i == 0)
        {
            specific_policy = ParallelismPolicy::All;
        }
        if (specific_policy != ParallelismPolicy::None)
        {
            selected_involved_dimensions = decode_involved_dimensions(specific_policy, model_parallel_npu_group);
        }
        else
        {
            selected_involved_dimensions = general_involved_dimensions;
        }
        // 每一行就是一个层的信息，创建一个 Layer 对象并存储到 layers 数组中。
        Layer *l = new Layer(id, i, generator, this, fp_compute_time * generator->compute_scale, fp_type, fp_group_type,
                             fp_comm_size * generator->comm_scale, selected_involved_dimensions["fwd"], ig_compute_time * generator->compute_scale,
                             ig_type, ig_group_type, ig_comm_size * generator->comm_scale, selected_involved_dimensions["ig"],
                             wg_compute_time * generator->compute_scale, wg_type, wg_group_type, wg_comm_size * generator->comm_scale,
                             selected_involved_dimensions["wg"], wg_update_time, specific_policy);
